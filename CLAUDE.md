...
- `docs/教育資料/AIによる事務処理・プログラミング自動化_環境構築ガイド.md` — 非エンジニア向けの WSL + VSCode + Git 導入手順書（Phase 1: git clone 直前まで）
- `docs/教育資料/AIによる事務処理・プログラミング自動化_ハンズオン.md` — git clone 後の `setup-*.sh` 実行と AI ツール起動までのハンズオン手順書（Phase 2）
