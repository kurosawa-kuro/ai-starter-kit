...
install_codex() {
  log "Codex CLI (@openai/codex) をグローバルインストールします。"
  npm install -g @openai/codex

  if command -v codex >/dev/null 2>&1; then
    log "codex コマンドが利用可能になりました: $(command -v codex)"
    codex --version || true
  else
    warn "codex コマンドが PATH に見つかりません。ターミナルを開き直してから再度確認してください。"
  fi
}
...
============================================================
 Codex CLI のセットアップが完了しました 🎉
============================================================

【次にやること】

 1. 一度 WSL のターミナルを開き直してください。
    （nvm / node / codex のパスを反映するため）
 ...
 3. 起動:
    ...
    初回起動時は OpenAI アカウントでのサインイン、または
    OPENAI_API_KEY の設定が必要です。画面の指示に従ってください。
