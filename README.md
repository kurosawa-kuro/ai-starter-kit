...
仕組みも理解しながら進めたい場合は 2 段階のドキュメントを順に読む:

| 段階 | 内容 | ドキュメント | 所要時間 |
|---|---|---|---|
| Phase 1 | WSL / VSCode / Git を Windows に手動セットアップ | [`01_環境構築ガイド.md`](./docs/教育資料/01_環境構築ガイド.md) | 30〜60 分 |
| Phase 2 | git clone → `src/scripts/setup-*.sh` 実行 → AI 起動 | [`02_ハンズオン.md`](./docs/教育資料/02_ハンズオン.md) | 10〜15 分 |
| 日常リファレンス | WSL 操作で詰まったとき | [`03_WSL操作.md`](./docs/教育資料/03_WSL操作.md) | 必要時 |

### 最短実行の例（Phase 1 が完了している前提）
...
| ファイル | 役割 |
|---|---|
| [`00_クイックスタート.md`](./docs/教育資料/00_クイックスタート.md) | **15 分で AI を動かす最短ルート（最初はこれ）** |
